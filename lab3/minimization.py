...
def minimization(struct: tuple[tuple[tuple]]):
    minimized = first_minimization(struct)
    if len(minimized) == 1:
        return minimized
    while True:
        old_data = minimized
...
def beautiful_minimization(minimiz: tuple[tuple[str]], is_sdnf=True):
    res = []
    for i in range(len(minimiz)):
        ...
        res.append('(' + (' ∧ '.join(inner) if is_sdnf else ' 🇻 '.join(inner)) + ')')
    return ' 🇻 '.join(res) if is_sdnf else ' ∧ '.join(res)
